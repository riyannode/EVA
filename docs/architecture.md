...
Runtime inputs use sorted compact JSON in user messages and are explicitly untrusted. The critic explains observed traces and cannot replace deterministic failure codes. Mutation must retain its parent category and supplied difficulty; code assigns provenance and identifiers. Repair receives invalid output and its error type, with exactly one repair attempt before fallback. Prompt strings are runtime instructions, the necessary exception to the source prose restriction.
