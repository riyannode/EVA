...
function OrderEvidence({ episodes }: { episodes: Episode[] }) {
  ...
  if (!traces.length) return <span className="analysis-muted">No order evidence</span>;
  const fields = ["orderId", "clientOid", "orderStatus", "executedQty", "avgPrice", "fee", "feeCurrency"];
  const values = traces.map(trace => {
    const data = trace.result.data;
    if (!data || typeof data !== "object" || Array.isArray(data)) return null;
    return Object.fromEntries(fields.filter(field => field in data).map(field => [field, data[field as keyof typeof data]]));
  }).filter(value => value && Object.keys(value).length);
  return values.length ? <Payload value={values} /> : <span className="analysis-muted">Order detail fields unavailable</span>;
}
...
export default function Analysis({ runs, runId, run, episodes, score, metrics, weaknesses, verification, onRunChange }: AnalysisProps) {
  ...
  return <div className="analysis-page" style={selectedStationStyle}>
    ...
    {tab === "Overview" && <>
      ...
      <Panel title="Behavioral Performance"><div className="analysis-list">{behaviorMetrics.map(([label, primary, secondary]) => { const value = metrics?.[primary]; const secondaryValue = metrics?.[secondary]; const isRate = primary.endsWith("_rate") || primary === "takeover_accuracy"; return <div className="analysis-row" key={label}><span>{label}</span><Value value={isRate ? rate(value as number | null | undefined) : count(value as number | null | undefined)} tone={value !== null && value !== undefined && value > 0 ? "warn" : ""} /><small>{isRate ? `${count(secondaryValue as number | null | undefined)} count` : "count"}</small></div>; })}</div></Panel>
