...
export default function Analysis({ runs, runId, run, episodes, score, metrics, weaknesses, verification, onRunChange }: AnalysisProps) {
  ...
  return <div className="analysis-page analysis-report">
    ...
    {!run ? <div className="analysis-empty-inline">Select an evaluation to see its results.</div> : <>
      ...
      {view === "Test details" && <section className="test-workspace panel"><aside className="test-index"><h2>Decision Trace</h2><ScrollArea><div className="test-index-list">{episodes.map(episode => <button key={episode.id} aria-pressed={selected?.id === episode.id} onClick={() => setEpisodeId(episode.id)}><span className={`oracle-dot ${episode.result.toLowerCase()}`} /><span><strong>Test {episode.number}</strong><small>{readable(episode.category)}</small></span><span>{readable(episode.result)}</span></button>)}</div></ScrollArea></aside>{selected ? <TestDetail episode={selected} /> : <p className="analysis-empty-inline">No test results recorded.</p>}</section>}
